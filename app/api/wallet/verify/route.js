...
export async function POST(req) {
  try {
    ...
    const tx = await getWalletTx(reference);
    ...
    if (result.data.status !== "success") {
      await updateWalletTxStatus(reference, "failed");
      return NextResponse.json({ error: "Payment not successful" }, { status: 400 });
    }

    await updateWalletTxStatus(reference, "success");
    const user = await adjustWalletBalance(tx.user_id, tx.amount);
