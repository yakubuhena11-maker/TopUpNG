...
import { getUserByPhone, createUser, getUserByReferralCode, createReferral, adjustWalletBalance, getSetting } from "@/lib/db";
...
export async function POST(req) {
  try {
    ...
    if (await getUserByPhone(cleanPhone)) return NextResponse.json({ error: "An account with this phone already exists" }, { status: 409 });

    const referredByUser = ref ? await getUserByReferralCode(ref) : null;
    const user = await createUser({ id: uuidv4(), phone: cleanPhone, password: hashPassword(password), referral_code: generateReferralCode(), referred_by: referredByUser?.id || null });
    if (referredByUser) {
